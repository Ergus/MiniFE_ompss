...
namespace miniFE
{
	struct ElemData {
		...
		double grad_vals[Hex8::numGaussPointsPerDim *
		                 Hex8::numGaussPointsPerDim *
		                 Hex8::numGaussPointsPerDim *
		                 Hex8::numNodesPerElem *
		                 Hex8::spatialDim];

		double elem_node_coords[Hex8::numNodesPerElem*Hex8::spatialDim];
		...
		void compute_element_matrix_and_vector()
		{
			Hex8::diffusionMatrix_symm(elem_node_coords, grad_vals, elem_diffusion_matrix);
			Hex8::sourceVector(elem_node_coords, grad_vals, elem_source_vector);
		}
	...
	struct ElemDataPtr {
		...
		double elem_node_coords[(Hex8::numNodesPerElem*(Hex8::spatialDim+1))/2];
		double* elem_diffusion_matrix;
		double* elem_source_vector;
