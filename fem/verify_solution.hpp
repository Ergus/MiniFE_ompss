...
int verify_solution(const simple_mesh_description &mesh,
                    const VectorType& x, double tolerance,
                    bool verify_whole_domain = false)
{
	...
	for(size_t i = 0; i < rows.size(); ++i) {
		double computed_soln = x.coefs[rows[i]];
	...
	double local_max_err = max_error.err;
